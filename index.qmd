...
::: notes
- Co-operators, c'est une coopérative d'assurance de dommage fondée en 1945 par un groupe de fermiers qui ne trouvaient pas de produit d'assurance convenable pour leurs fermes.
...
::: notes
- Maintenant, je vais vous parler d'un projet que j'ai fait dans l'équipe de recherche. C'est en fait le premier projet qui m'a été assigné quand j'ai commencé à tavailler.
:::
...
- Par exemple, quel code doit-on assigner à l'entreprise décrite par [Small place where they serve coffee and food such as pastries]{style='color: #FFFFFF; background-color: #007d57; border-radius: 5px; padding: 2px 5px;'}?
:::
...
::: {style='font-size: 25px; position: absolute; top: 315px; left: 0px;'}
IBC Codes
...
::: notes
- Le modèle de langage qu'on a utilisé (comme beaucoup d'autres modèles de langage) est un modèle d'embedding.
- C'est quoi un embedding? C'est simplement une chaine de caractères encodée sous forme d'un vecteur.
- Ça peut être n'importe quelle chaine de caractères: un mot, une phrase, un document, etc.
- Donc un modèle d'embedding, c'est un modèle qui permet de transformer du texte en nombres.
- Important parce qu'un ordinateur ne peut pas traiter du texte, seulement des nombres.
- Un modèle d'embedding, c'est une fonction qui prend en entrée du texte et qui renvoie un vecteur en n dimensions.
- Et comme vous le savez, un vecteur en n dimensions définit un point dans un espace en n dimensions qu'on appelle l'espace d'embedding.
:::
...
::: notes
- Un bon modèle d'embeddings, c'est un modèle qui va placer des mots (ou des chaines de caractères) de sens similaire proches dans l'espace d'embeddings. À l'inverse, les mots de sens non-similaire doivent être éloignés.
...
- Ici embedding très simple en 2D mais habituellement, on a beaucoup plus de dimensions que ça.
:::
...
- Donc on peut s'imaginer que la dimension 1 correspond au degré de « vie », la dimension 2 correspond au degré de « félinité », la troisième au degré d'humanité, etc.
- Par exemple, le chat et le chaton ont un haut degré de félinité, tandis que la maison a un faible degré de félinité.
- En pratique par contre, on ne peut pas vraiment associer chaque dimension à un concept qu'on peut interpréter.
...
On génère d'abord des [**paires positives**]{style='color: #007d57;'} et des [**paires négatives**]{style='color: #650b22;'}.
...
::: notes
- La première étape est de se créer un jeu de données étiqueté. On va se créer autant de paires positives que de paires négatives.
...
<br>
[Small place where they serve coffee and food such as pastries]{style='color: #FFFFFF; background-color: #007d57; border-radius: 5px; padding: 2px 5px;'}
...
::: {.ol-font-size-medium}
1. On obtient un **embedding** représentant la **description d'entreprise cible** en utilisant notre modèle fine-tuné.
2. On a préalablement fait de même pour les **136 000 entreprises** de notre jeu de données.
3. On calcule la **distance** entre l'**entreprise cible** et toutes les **entreprises de notre jeu de données**.
4. On trouve les **k entreprises les plus proches** (algorithme des $k$ plus proches voisins).
4. On suggère les **codes les plus fréquents** dans les $k$ entreprises voisines.
:::
